...
explictly written to control Ronin Gimbals, but shoud work generally
...
void Ronin_SBUS::begin(int commandPin)
{
  ...
  int16_t loc_channelValues[18]   = {1023,1023,1023,1023,352,176,352,1023,1023,1023,1023,1023,1023,1023,1023,1023,0,0};
  ...
  memcpy(channelValues,loc_channelValues,18);
}

void Ronin_SBUS::SetValue(uint8_t ch, int16_t position) 
{
  ...
  if ((ch>0)&&(ch<=16)) 
  {
    ...
    channelValues[ch-1] = position; //expects a non zero starting index to the channel
  }
}

void Ronin_SBUS::Send(void)
{
  //send data over serial port
  SERIALPORT.write(sbusData, 25); //according to docs for Serial we can send the array along as is without a loop
}

void Ronin_SBUS::Update(void) 
{
  ...
  {
    if (channelValues[ch] & (1<<bit_in_servo)) //bitwise AND to check if the correct servo databit is set to 1
    {
